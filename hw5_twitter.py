...
def construct_unique_key(baseurl, params):
    ...
    param_strings.sort()
    unique_key = baseurl + cntr + cntr.join(param_strings)
    return unique_key
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for t in tweets:
        ...
        for n in hashtags:
            if n["text"].lower() != hashtag_to_ignore:
                if n["text"] in tmp_dict.keys():
                    tmp_dict[n["text"]] += 1
                else:
                    tmp_dict[n["text"]] = 1
    
    for cnt in tmp_dict:
        if tmp_dict[cnt] > max_count:
            max_count = tmp_dict[cnt]
            pop_word = cnt
